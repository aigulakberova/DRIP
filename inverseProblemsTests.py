...
train_data = torchvision.datasets.STL10(root=path, download=True, transform=torchvision.transforms.ToTensor())
train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=num_workers)
test_data = torchvision.datasets.STL10(root=path, download=True, transform=torchvision.transforms.ToTensor(),
                                       split="test")
...
def test():
    ...
    with torch.no_grad():
        for batch_idx, (img, lbl) in enumerate(test_loader):
            ...
            alpha = 0 # 25 + 25 * torch.rand((1)).to(device)
